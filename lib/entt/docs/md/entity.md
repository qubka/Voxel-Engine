...
the user code base, nor to control its main frame or process scheduling.<br/>
...
* It's iterable and therefore users can easily walk through the list of entities
  by means of a range-for frame or the `each` member function.
...
To iterate a view, either use it in a range-for frame:
...
To iterate a runtime view, either use it in a range-for frame:
...
To iterate groups, either use them in a range-for frame:
...
Use a common range-for frame and get components directly from the view or move
