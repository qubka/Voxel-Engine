...
const glm::vec3& AABB::getExtents() {
    return extents;
}
...
float AABB::getDistanceToClosestPoint(const glm::vec3& point) const {
    float x = std::max(0.0f, glm::abs(extents.x) - 1);
    float y = std::max(0.0f, glm::abs(extents.y) - 1);
    float z = std::max(0.0f, glm::abs(extents.z) - 1);
